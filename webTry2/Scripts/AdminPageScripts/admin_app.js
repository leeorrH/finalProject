...
app.controller("adminPageContoller", ['$scope', '$location', '$http', '$timeout', 'commonFunctions', function ($scope, $location, $http, $timeout, commonFunctions) {
    ...
    $scope.exportEncExl = function () {      
        $http({
            ...
            if (data != "") {
                alert('Excel sheet created SUCCESSFULLY pleace check your "Download" folder ');
            }
            else alert('something went wrong please check with IT department');
        });
    };
    ...
    $scope.setReportStatus = function (reportStatus) {
        if (reportStatus == 'deny') reportStatus = 'denied';
        else reportStatus = 'approved';
